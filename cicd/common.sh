...
spawn_docker_host() {
  ...
  if [[ "$dtype" == "loxilb" ]]; then
    ...
    if [[ ! -z ${ka+x} ]]; then
      ...
      echo "docker exec -dt $dname /root/loxilb-io/loxilb/loxilb $bgp_opts $cluster_opts $extra_opts"
    fi
...
config_lb_state() {
    llbIP=$(docker inspect --format='{{.NetworkSettings.IPAddress}}' $1)
    State=$(echo "$2" | tr '[:lower:]' '[:upper:]')
        json_data=$(jq -n \
        --arg instance "llb-inst0" \
        --arg state "$State" \
        --arg vip "0.0.0.0" \
        '{instance: $instance, state: $state, vip: $vip}')
    
    curl -X 'POST' "http://$llbIP:11111/netlox/v1/config/cistate" \
